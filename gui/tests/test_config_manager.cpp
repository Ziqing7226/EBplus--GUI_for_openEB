...
TEST(ConfigManagerAlgoState, RoundTripAlgoParams) {
    ...
    // Inspect the serialized JSON before reloading.
    {
        ...
        EXPECT_EQ(params.value("fpn_alpha").toString(), "0.5");
    ...
    EXPECT_NE(inst2->get_param("fpn_alpha"), "0.5");  // default before load
    ...
    EXPECT_EQ(inst2->get_param("fpn_alpha"), "0.5");
...
TEST(ConfigManagerAlgoState, ApplyStateCachesParamsForNonLiveAlgos) {
    ...
    QJsonObject entry;
    QJsonObject params;
    params["fpn_alpha"] = QStringLiteral("0.9");
    entry["params"] = params;
    entry["enabled"] = true;
