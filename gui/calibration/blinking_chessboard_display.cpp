...
// wizard's capture window must cover at least one full cycle so every black
...
BlinkingChessboardDisplay::BlinkingChessboardDisplay(QWidget* parent) : QWidget(parent) {
    ...
    // We paint every pixel ourselves (white fill + black squares), so Qt can
...
void BlinkingChessboardDisplay::recompute_layout() {
    ...
    // Frame A: chessboard — black squares on white background (checkerboard
    // parity: (col+row) even = black). Frame B: blank white. Both rendered with
    // batched solid QPainter primitives (drawRects), keeping the QPixmap on the
    // native blit path (no QImage full-widget CPU→GPU upload — see project
    // memory on the maximized-window preview stutter).
    {
        pixmap_a_ = QPixmap(size());
        pixmap_a_.fill(Qt::white);
